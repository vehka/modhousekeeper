...
## Phase 3 (done 2026-05-31)

On-device testing found install failures. Root cause in every case was **stale
data in `mods.lua`**, not the install code (install_mod correctly requires
`<dir>/lib/mod.lua` after clone). Fixes:

- **arcify** removed -- `mimetaur/arcify` is a Lua *library* (`lib/arcify.lua`,
  no `lib/mod.lua`), so it installs but never shows in the MODS menu.
- **broadcast** URL corrected `schollz/norns-broadcast` (404) -> `schollz/broadcast`.
- **receiver** URL corrected to `markijzerman/receiver` (the schollz ones 404).
- **iiitoii** URL corrected `frederickk/iiitoii` (404) -> `andr-ew/iiitoii`.
- **semiconductor** URL corrected `jaseknighter/semiconductor` (404) ->
  `jaseknighter/norns_semiconductor`.
- **midigrid** miker2049 alt dropped -- that fork is a library, no `lib/mod.lua`.
- **ndi-mod** removed -- real mod but `mod.lua` is at `mod/lib/mod.lua` and it
  needs a native NDI build, so a plain `git clone` can't install it.

**`tools/validate_mods.lua`** added: a maintenance util (NOT loaded by the mod)
that loads `mods.lua` and checks every entry (+ alts) on GitHub for `lib/mod.lua`
on its default branch. Prefers the `gh` CLI (auth -> high rate limit), falls back
to unauthenticated curl. Run `lua tools/validate_mods.lua` -- exit 0 = all valid,
1 = problems listed. After the fixes above it passes clean. Re-run it whenever
editing `mods.lua`.

Note: the catalog-refresh popup reporting "350 entries" is cosmetic (that is the
community.json size) and was not flagged by the user as a problem; left as-is.
Scrolling, "browse community mods", and install-vs-enable indicators were
considered but the user confirmed they are NOT needed.
